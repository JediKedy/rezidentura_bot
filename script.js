...
async function init() {
    try {
        ...
        document.getElementById('content').innerHTML = `<div class="p-4 bg-red-100 text-red-700 rounded-lg">Fayllar yüklənmədi.</div>`;
    }
}
...
function renderMenu(data, title) {
    ...
    Object.keys(data).forEach(key => {
        ...
        btn.className = "w-full flex items-center justify-between p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm hover:border-blue-400 dark:hover:border-blue-500 transition-all active:scale-[0.98]";
        btn.innerHTML = `
...
async function loadQuiz(shuffle) {
    ...
    quizState.index = 0; quizState.correct = 0; quizState.incorrect = 0;
    showQuestion();
}
...
function showQuestion() {
    ...
    container.innerHTML = `
        <div class="space-y-4">
            ${q.image ? `<img src="${q.image}" class="w-full rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">` : ''}
            <div class="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p class="text-lg leading-relaxed font-medium">${q.question}</p>
            </div>
    ...
    q.options.forEach((opt, idx) => {
        const btn = document.createElement('button');
        btn.className = "flex items-start gap-3 w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-left transition-all hover:bg-slate-50 dark:hover:bg-slate-800";
        btn.innerHTML = `
...
function selectOption(btn, isCorrect) {
    const allBtns = document.querySelectorAll('#options-box button');
